...
export default function PinLockScreen() {
  ...
  // التحقق التلقائي عند اكتمال 4 خانات
  useEffect(() => {
    if (autoVerify && pin.length >= 4) {
      handleVerify(pin);
    }
  }, [pin, autoVerify, handleVerify]);
    ...
    <div className="fixed inset-0 z-[9999] bg-white flex items-center justify-center" dir="rtl">
      <div className="w-full max-w-xs px-6 flex flex-col items-center gap-8">
        ...
        >
          ...
          {!autoVerify && pin.length >= 4 && (
            <p className="text-xs text-gray-400 mt-1">اضغط ✓ للتحقق</p>
          )}
        </motion.div>
