...
import {
  ...
  MessageCircle, CheckCircle2, Loader2, Phone, Hash
} from 'lucide-react';
            ...
            >
              <div className="bg-white/80 backdrop-blur-2xl rounded-[2.5rem] p-6 sm:p-8 shadow-xl border border-gray-100 relative overflow-hidden">
                ...
                <div className="relative flex bg-gray-100 rounded-2xl p-1.5 mb-6 border border-gray-200">
                  ...
                  <button type="button" onClick={() => { setIsLogin(true); setAuthMode('email'); setError(''); setSuccess(''); }} className={`relative z-10 flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${isLogin && authMode === 'email' ? 'text-white' : 'text-gray-500 hover:text-gray-700'}`}>
                    <Mail className="w-4 h-4 inline ml-1" /> تسجيل
                  </button>
                  <button type="button" onClick={() => { setIsLogin(false); setError(''); setSuccess(''); }} className={`relative z-10 flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${!isLogin ? 'text-white' : 'text-gray-500 hover:text-gray-700'}`}>
                    <Users className="w-4 h-4 inline ml-1" /> جديد
                  </button>
                  <button type="button" onClick={() => { setIsLogin(true); setAuthMode('phone'); setError(''); setSuccess(''); }} className={`relative z-10 flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${isLogin && authMode === 'phone' ? 'text-white' : 'text-gray-500 hover:text-gray-700'}`}>
                ...
                {isLogin && authMode === 'email' && (
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button type="button" onClick={onForgotPassword} className="text-xs text-purple-500 hover:text-purple-700 font-medium text-left transition-all hover:underline active:scale-95 origin-left" disabled={loading}>نسيت كلمة المرور؟</button>
                    ...
                    <Button type="submit" disabled={loading} className="w-full h-14 bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-400 hover:to-blue-400 text-white rounded-xl font-bold text-base shadow-lg shadow-purple-200/50 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 group/btn">
                      {loading ? <Loader2 className="w-5 h-5 animate-spin"/> : <>تسجيل الدخول<ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform"/></>}
                    </Button>
                  </form>
                )}
                ...
                {!isLogin && (
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {success && <div className="bg-emerald-50 border border-emerald-200 text-emerald-600 text-sm p-3 rounded-xl flex items-center gap-2"><CheckCircle2 className="w-4 h-4 shrink-0"/>{success}</div>}
                    <Button type="submit" disabled={loading} className="w-full h-14 bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-400 hover:to-blue-400 text-white rounded-xl font-bold text-base shadow-lg shadow-purple-200/50 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">
                      {loading ? <Loader2 className="w-5 h-5 animate-spin"/> : <>إنشاء حساب<ArrowRight className="w-4 h-4"/></>}
                    </Button>
                  </form>
                )}
