...
export default function AuthScreen({ onLogin, onForgotPassword }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);      
      setError(['auth/invalid-credential','auth/wrong-password'].includes(err.code) ? 'البريد أو كلمة المرور غير صحيحة' : 
    ...
    try {
      ...
      if (!(await getDoc(ref)).exists()) {
        await setDoc(ref, { 
          ...
          username: '', status: 'online', lastSeen: serverTimestamp(),           loginDates: [new Date().toISOString().split('T')[0]], loginCount: 1, 
          settings: { fontSize: 'medium', fontFamily: 'tajawal', muteMicOnJoin: false, speakerDefault: false }, 
          contacts: [], blockedUsers: [], createdAt: serverTimestamp() 
        });
      }
          ...
          <div className="bg-white/[0.03] backdrop-blur-2xl rounded-[2.5rem] p-6 sm:p-8 shadow-2xl border border-white/10 animate-in fade-in zoom-in-95 duration-700 relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative group/input">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white p-1 active:scale-90" disabled={loading}>{showPassword ? <EyeOff className="w-5 h-5"/> : <Eye className="w-5 h-5"/>}</button>
              </div>
              ...
              {!isLogin && <div className="relative group/input">
                ...
                <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white p-1 active:scale-90" disabled={loading}>{showConfirm ? <EyeOff className="w-5 h-5"/> : <Eye className="w-5 h-5"/>}</button>
              </div>}
            ...
            <Button type="button" onClick={handleGoogleSignIn} disabled={loading} className="w-full h-14 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl font-bold text-base transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 group/google">
