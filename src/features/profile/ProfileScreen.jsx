...
import { LogOut, Mail, Lock, User, Trash2, Shield, ChevronRight, Pencil, Smartphone, Sparkles, X, Crown } from 'lucide-react';
          ...
          >
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                <motion.button whileTap={{ scale: 0.95 }} onClick={onClose} className="flex-1 h-11 rounded-xl border-2 border-gray-200 bg-white hover:bg-gray-50 font-medium transition-all">إلغاء</motion.button>
                <motion.button whileTap={newName.trim() ? { scale: 0.95 } : {}} onClick={handleSave} disabled={!newName.trim()} className="flex-1 h-11 rounded-xl bg-gradient-to-r from-purple-600 to-blue-500 hover:from-purple-700 hover:to-blue-600 text-white font-bold shadow-md shadow-purple-500/20 transition-all disabled:opacity-50">حفظ</motion.button>
              </div>
            </div>
          </motion.div>
        </motion.div>
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-purple-50 via-white to-blue-50 pb-32 text-right" dir="rtl">
      ...
      >
        <div className="flex items-center justify-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-blue-500">الملف الشخصي</h1>
        </div>
          ...
          <AnimatePresence mode="wait">
            {!showDelete ? (
              ...
              <motion.div key="confirm" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white border-2 border-red-200 rounded-2xl p-6 space-y-5 text-center shadow-lg">
                ...
                <div><p className="text-red-800 font-bold text-lg mb-1">هل أنت متأكد؟</p><p className="text-sm text-gray-500">هذا الإجراء لا يمكن التراجع عنه.</p></div>
                <div className="flex gap-3">
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => setShowDelete(false)} className="flex-1 h-11 rounded-xl border-2 border-gray-200 bg-white hover:bg-gray-50 font-medium transition-all">إلغاء</motion.button>
                  <motion.button whileTap={{ scale: 0.9 }} onClick={handleDelete} disabled={deleting} className="flex-1 h-11 rounded-xl bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-bold shadow-md shadow-red-200 disabled:opacity-50 transition-all">{deleting ? 'جارٍ...' : 'تأكيد الحذف'}</motion.button>
                </div>
      ...
      <NameChangeModal open={showNameModal} onClose={() => setShowNameModal(false)} currentName={displayName} onSave={(name) => onUpdateProfile?.({ displayName: name })} />
    </div>
  );
}